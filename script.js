...
let listHTML3 = document.querySelector('.carousel3 .list3');
let seeMoreButtons3 = document.querySelectorAll('.seeMore3');
let backButton3 = document.getElementById('back3');
let autoSlideDelay3 = 5000; // 3 seconds interval for automatic slide
...
let unAcceppClick3;
const showSlider3 = (type) => {
    nextButton3.style.pointerEvents = 'none';
    prevButton3.style.pointerEvents = 'none';

    carousel3.classList.remove('next3', 'prev3');
    let items = document.querySelectorAll('.carousel3 .list3 .item3');
    if(type === 'next3'){
        listHTML3.appendChild(items[0]);
        carousel3.classList.add('next3');
    }else{
        listHTML3.prepend(items[items.length - 1]);
        carousel3.classList.add('prev3');
    }
    clearTimeout(unAcceppClick3)
    unAcceppClick3 = setTimeout(()=>{
        nextButton3.style.pointerEvents = 'auto';
        prevButton3.style.pointerEvents = 'auto';
    }, 2000)
}
seeMoreButtons3.forEach((button) => {
    button.onclick = function(){
        carousel3.classList.remove('next3', 'prev3');
        carousel3.classList.add('showDetail3');
    }
});
backButton3.onclick = function(){
    carousel3.classList.remove('showDetail3');
}

let autoSlideInterval3 = setInterval(() => {
  showSlider3('next3');
}, autoSlideDelay); // 5000ms = 3 seconds for automatic slide transition

 

let carouselItems3 = document.querySelectorAll('.carousel3 .list3 ');

// Function to stop auto-slide
const stopAutoSlide3 = () => {
    clearInterval(autoSlideInterval3);
};

// Function to restart auto-slide
const startAutoSlide3 = () => {
    clearInterval(autoSlideInterval3); // Ensure no duplicate intervals
    showSlider3('next3');
...
carouselItems3.forEach((item) => {
    item.addEventListener('mouseenter', stopAutoSlide3);
    item.addEventListener('mouseleave', startAutoSlide3);
});
/*--------------------------------------------------------------------------------------------------------*/
let nextButton4 = document.getElementById('next4');
let prevButton4 = document.getElementById('prev4');
let carousel4 = document.querySelector('.carousel4');
let listHTML4 = document.querySelector('.carousel4 .list4');
let seeMoreButtons4 = document.querySelectorAll('.seeMore4');
let backButton4 = document.getElementById('back4');
let autoSlideDelay4 = 5000; // 3 seconds interval for automatic slide
nextButton4.onclick = function(){
    showSlider4('next4');
}
prevButton4.onclick = function(){
    showSlider4('prev4');
}
 
let unAcceppClick4;
const showSlider4 = (type) => {
    nextButton4.style.pointerEvents = 'none';
    prevButton4.style.pointerEvents = 'none';
 
    carousel4.classList.remove('next4', 'prev4');
    let items4 = document.querySelectorAll('.carousel4 .list4 .item4');
    if(type === 'next4'){
        listHTML4.appendChild(items4[0]);
        carousel4.classList.add('next4');
    }else if (type === 'prev4'){
        listHTML4.prepend(items4[items4.length - 1]);
        carousel4.classList.add('prev4');
    }
    clearTimeout(unAcceppClick4);
    unAcceppClick4 = setTimeout(() => {
        nextButton4.style.pointerEvents = 'auto';
        prevButton4.style.pointerEvents = 'auto';
    }, 6000);
}
 
seeMoreButtons4.forEach((button) => {
    button.onclick = function(){
        carousel4.classList.remove('next4', 'prev4');
        carousel4.classList.add('showDetail4');
    }
});
backButton4.onclick = function(){
    carousel4.classList.remove('showDetail4');
}
 
// Interval for auto-sliding
let autoSlideInterval4 = setInterval(() => {
  showSlider4('next4');
}, autoSlideDelay4); // 5000ms = 3 seconds for automatic slide transition
 
// Select all items4 in the carousel
let carouselItems4 = document.querySelectorAll('.carousel4 .list4');
 
// Function to stop auto-slide
const stopAutoSlide4 = () => {
    clearInterval(autoSlideInterval4);
};
 
// Function to restart auto-slide
const startAutoSlide4 = () => {
    clearInterval(autoSlideInterval4); // Ensure no duplicate intervals
    showSlider4('next4');
...
// Add event listeners for each item
carouselItems4.forEach((item4) => {
    item4.addEventListener('mouseenter', stopAutoSlide4);
    item4.addEventListener('mouseleave', startAutoSlide4);
});
